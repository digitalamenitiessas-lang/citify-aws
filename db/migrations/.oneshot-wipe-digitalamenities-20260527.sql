-- ============================================================================
-- ONE-SHOT WIPE — NO ES UNA MIGRACIÓN ESTRUCTURAL
-- ============================================================================
-- Resetea TODOS los datos operativos del admin digitalamenitiessas@gmail.com,
-- dejando solo su profile + sus administraciones (vacías) + memberships.
-- Borra también vecinos vinculados a sus buildings y los buildings mismos.
--
-- Scope confirmado por usuario (2026-05-27): "Total + borrar vecinos".
...
BEGIN
  ...
  IF v_admin_ids IS NULL THEN
    RAISE NOTICE '[wipe] El profile no es miembro de ninguna administración. Solo limpio building_id.';
  ...
  -- ─── 3. Properties y buildings de esas administraciones ─────────────────
  ...
  -- ─── 4. Borrar capa iadmin (de hojas hacia la raíz) ─────────────────────
  ...
  -- ─── 5. Borrar capa building (announcements, complaints, info) ──────────

  IF array_length(v_building_ids, 1) > 0 THEN

    -- announcement_reads (puede no existir si no se usó)
    BEGIN
  ...
  -- ─── 8. Limpiar building_id del admin si quedó colgando ────────────────
  ...
  RAISE NOTICE '[wipe] VERIFY: memberships conservadas para el admin = %', v_count;
