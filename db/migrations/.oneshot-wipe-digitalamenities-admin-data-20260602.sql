...
-- ============================================================================
-- Resetea los DATOS DE ADMINISTRACIÓN DE CONSORCIO cargados por el admin
-- digitalamenitiessas@gmail.com pero CONSERVA:
--   • profile del admin + role grants (iadmin_role_grants)
--   • iadmin_managed_properties (vínculo administración ↔ edificio)
--   • buildings y consorcio_assignments
--
-- Borra (si existen las tablas): unidades, holders (contactos), vecinos
-- (profiles con login del edificio), unit_profile_memberships,
-- building_information, gastos + documentos + extracciones IA, providers,
-- liquidaciones + items + share tokens, payments, bank_movements,
-- cash_accounts, accounting_periods, reminders, notifications, audit_logs,
-- anuncios + reads y reclamos (cases/messages/events/mentions/reasons).
--
...
--
-- NOTA: holders.profile_id (puente contacto ↔ login) se elimina junto con
-- iadmin_unit_holders, así que la unificación holder/usuario queda limpia.
-- ============================================================================
...
BEGIN
  ...
  SELECT count(*) INTO v_count FROM public.iadmin_unit_holders
   WHERE unit_id IN (SELECT id FROM public.iadmin_units WHERE managed_property_id = ANY(v_property_ids));
  RAISE NOTICE '[wipe] VERIFY: holders restantes = % (esperado 0)', v_count;
