...
export function renderWelcomeEmail(input: WelcomeInput): {
  ...
  const contextLine = input.buildingName
    ...
    : `Tu cuenta en Citify quedó activa con rol de <strong>${escapeHtml(roleLabel)}</strong>.`
  ...
  const { html, text } = renderEmailLayout({
    preheader: input.temporaryPassword
      ? 'Tu cuenta Citify está lista. Adentro están tus credenciales.'
      : 'Tu cuenta Citify está lista.',
    title: '¡Bienvenido/a a Citify!',
    intro: input.buildingName
      ? `Te sumaron a ${input.buildingName} en Citify.`
      : input.businessName
      ? `Sumaron tu negocio ${input.businessName} a Citify.`
      : 'Tu cuenta acaba de activarse.',
    bodyHtml,
    ctaLabel: 'Ingresar a Citify',
  ...
  return {
    subject: input.buildingName
      ? `Bienvenido/a a ${input.buildingName} en Citify`
      : input.businessName
      ? `Tu negocio ${input.businessName} ya está en Citify`
      : 'Tu cuenta Citify está lista',
    html,
    text,
  }
