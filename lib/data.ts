...
function mapPromotionFromPostgresRow(row: Awaited<ReturnType<typeof getPublicPromotionsFromPostgres>>[number]): Promotion {
  ...
  return {
    ...
    businessLogoUrl: row.business_logo_path?.startsWith('public/') ? buildPublicS3Url(row.business_logo_path) : null,
    title: row.title,
    description: row.description,
    discount: row.discount,
    category: row.category,
...
export async function getSuperAdminDashboardData(): Promise<SuperAdminDashboardData> {
  ...
  const allPromotionsRaw = promotionsRows.map((row): Promotion => ({
    ...
    businessLogoUrl: (row as any).business_logo_path?.startsWith('public/') ? buildPublicS3Url((row as any).business_logo_path) : null,
    title: row.title,
    description: row.description,
    discount: row.discount,
    category: row.category ?? '',
