...
Recibiras encabezados de una planilla Excel y algunas filas de ejemplo. Tu tarea es identificar que columna corresponde a cada campo interno de CITIFY y devolver UNICAMENTE un JSON valido.
...
export async function inferInitialOccupancyMapping(input: {
  ...
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'HTTP-Referer': 'https://citify.app',
      'X-Title': 'CITIFY SuperAdmin Import',
    },
    body: JSON.stringify({
      model,
      temperature: 0,
